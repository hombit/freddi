...
public:
	...
	template <DiskIntegrationRegion Region> double flux_region(const Passband& passband) const {
	...
	inline double flux(const Passband& passband) const  { return flux_region<HotRegion>(passband); }
	double flux_star(double lambda, double phase) const;
	double flux_star(const Passband& passband, double phase) const;
	...
	double R_cooling_front(double r, double sigma_at_r) const;
