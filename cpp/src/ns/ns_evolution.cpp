...
//			new_F_in = F_dead() * m::pow<3>(R_dead() / R_m);
	...
	//@@::
	//current_.F_in +=  Mdot_in() * ( h()[first()] - std::pow(GM()*R_m_physical,0.5) ) ; 
 }
...
void FreddiNeutronStarEvolution::nonlinear_diffusion(const double tau) {
    ...
    double Fvis0 = abs(Fvis_start[first()]  - Mdot_in()*(h()[first()]-std::pow(GM()*R()[first()+1],0.5)));
    ...
    std::cout << "time=" << (FreddiState::current_.t)/86400. << " Mdot=" << Mdot_in() << " R_Magn=" << R_Alfven() <<std::endl;
    int bits = 10;
    std::pair<double, double> brent_result = boost::math::tools::brent_find_minima(
	[this, &Fvis_start, tau, Fvis0](double fin){
		...
		args().calc->tau, args().calc->eps,
						    fin*Fvis0, Mdot_outer_boundary(),
						    windA(), windB(), windC(),
						    wunc(),
						    h(), current_.F,
						    first(), last());
	    double Answer = (current_.F[first()] - Mdot_in()*(h()[first()]-std::pow(GM()*R_Alfven(),0.5)))/Fvis0;
// 	    double Answer_div = (current_.F[first()+1]-current_.F[first()])/Fvis0; 
	   
	
	    std::cout << " Mdot=" << Mdot_in() << " Answer=" << Answer 
	    << " R[first]=" << R()[first()] << " R_Magn=" << R_Alfven() 
	    << " fin=" << fin  <<  std::endl;
    ...
    nonlinear_diffusion_nonuniform_wind_1_2(
					    args().calc->tau, args().calc->eps,
					    brent_result.first*Fvis0, Mdot_out(),
					    windA(), windB(), windC(),
					    wunc(),
					    h(), current_.F,
					    first(), last());
